...
class SQLTemplate {
	...
	var $array_placeholders;   /**< \private List of array parameters */
	...
	function parse($sql_template) {
		...
		if (preg_match_all($named_fieldspattern, $sql_template, $fields)) {
			...
			foreach (array_keys($fields[0]) as $match)
			{
				$this->named_placeholders[] = array(
					'type' => SQLTemplate::column_type_from_string($fields[1][$match]),
					'var' => $fields[3][$match]
				);
				$this->array_placeholders[] = (boolean) $fields[2][$match];
			}
			...
			foreach (array_keys($fields[0]) as $match)
			{
				$this->placeholders[] = SQLTemplate::column_type_from_string($fields[1][$match]);
				$this->array_placeholders[] = (boolean) $fields[2][$match];
			}
	...
	function fill($args=null) {
		...
		if($this->named_mode) {
			...
			/* Exception: using named mode, but no placeholders
			 * to detect them: just remove the arguments
			 */
			if (count($this->placeholders == 0) && count($args) == 1 &&
					($args[0] instanceof Container || is_assoc_array($args[0])))
				$args = array();
		...
		for ($i = 0; $i < $numargs; $i++) {
			...
			if ($this->array_placeholders[$i]) {
				$value = $this->escape_field_array($fieldtype, $value, $argname);
			} else {
				$value = $this->escape_field($fieldtype, $value, $argname);
			}
	...
	/**
	 * Escape the values as an array based on fieldtype. Does validating
	 * and generates errors if invalid.
	 *
	 * \param $fieldtype
	 *	The primitive type of the field.
	 * \param $values
	 *	An array of values to escape with $fieldtype.
	 * \param $argname
	 *	The identifier for use in error messages.
	 *
	 * \return
	 *	A comma-separated list of escaped values.
	 */
	function escape_field_array($fieldtype, $values, $argname) {
		if (!is_array($values))
			trigger_error(sprintf('Invalid array value: "%s" on argument %s', $values, $argname),
					E_USER_ERROR);

		$values_escaped = array();
		foreach ($values as $idx => $value) {
			$values_escaped[] = $this->escape_field($fieldtype, $value, $argname.'['.$idx.']');
		}
		return join(', ', $values_escaped);
	}

	/**
	 * Escape the values based on fieldtype. Does validating and generates
	 * errors if invalid.
	 *
	 * \param $fieldtype
	 *	The type of the field.
	 * \param $value
	 *	The value to escape, must match the fieldtype.
	 * \param $argname
	 *	The identifier for use in error messages.
	 *
	 * \return
	 *	The escaped value.
	 */
	function escape_field($fieldtype, $value, $argname) {
		...
		switch ($fieldtype) {
			...
			case ANEWT_DATABASE_TYPE_BOOLEAN:
				...
				if (is_bool($value)) {
					$value = $this->db->escape_boolean($value);
					break;
				}
			...
			case ANEWT_DATABASE_TYPE_DATE:
				...
				if (is_string($value) && preg_match('/^\d{2,4}-\d{2}-\d{2}$/', $value)) {
					$value = $this->db->escape_date($value);
					break;
				}
			...
			case ANEWT_DATABASE_TYPE_TIME:
				...
				if (is_string($value) && preg_match('/^\d{2}:\d{2}(:\d{2})?$/', $value)) {
					$value = $this->db->escape_time($value);
					break;
				}
			...
			case ANEWT_DATABASE_TYPE_COLUMN:
				...
				if (is_string($value) && preg_match('/^([a-z0-9_-]+\.)*[a-z0-9_-]+$/i', $value)) {
					$value = $this->db->escape_column_name($value);
					break;
				}
			...
			case ANEWT_DATABASE_TYPE_TABLE:
				...
				if (is_string($value) && preg_match('/^([a-z0-9_-]+\.)*[a-z0-9_-]+$/i', $value)) {
					$value = $this->db->escape_table_name($value);
					break;
				}
