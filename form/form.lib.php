...
class AnewtForm extends Container
{
	 ...
	 */
	protected $_controls_by_name = array();

	/**
	 * Associative array to hold all form fieldsets by name.
	 */
	protected $_fieldsets_by_name = array();

	/**
	 * List of references to hidden controls.
	 */
	protected $_hidden_controls = array();

	/**
	 * List of invalid controls. Mainly for debugging.
	 */
	public $invalid_controls;
	...
	/**
	 * Seed the controls with default values. This will simply set the values
	 * of the control. See fill() and autofill() for filling the controls
	 * with user supplied values.
	 *
	 * Note that this method bypasses $control->fill($values) and directly
	 * calls $control->set('value', $values[$name]). This way disabled
	 * controls are also set.
	 *
	 * \param $values
	 *   Associative array or Container with a mapping from control names
	 *   to values.
	 * \param $keys
	 *   Optional array containing the keys to map.
	 *
	 * \see AnewtFormControl::fill()
	 */
	function seed_controls($values, $keys=null)
	{
		assert('is_object($values) || is_array($values)');
		if (is_array($values))
		{
			foreach (array_keys($this->_controls_by_name) as $name)
			{
				if (array_key_exists($name, $values))
					$this->_controls_by_name[$name]->set('value', $values[$name]);
			}
		}
		else
		{
			foreach (array_keys($this->_controls_by_name) as $name)
			{
				if ($values->is_set($name))
					$this->_controls_by_name[$name]->set('value', $values->get($name));
			}
		}
	}
	 ...
	 * this.
	 *
	 * $this->invalid_controls will contain an array with the names of all
	 * invalid controls, should validation fail. Note that this array will
	 * be reset is_valid()
	 *
	 * \param $values
	...
	function fill($values)
	{
		...
		$out = true;
		$this->invalid_controls = array();
		foreach (array_keys($this->_controls_by_name) as $name)
		{
			$valid = $this->_controls_by_name[$name]->fill($values);
			if (!$valid) {
				$this->invalid_controls[] = $name;
			}
			$out = $valid && $out;
		}
		return $out;
	...
	function get_control_value($name)
	{
		...
		assert('array_has_key($this->_controls_by_name, $name); // control '.$name.' must exist');
	 ...
	 *
	 * $this->invalid_controls will contain an array with the names of all
	 * invalid controls, should validation fail.
	 *
	...
	function is_valid()
	{
		...
		foreach (array_keys($this->_controls_by_name) as $name)
		{
			$valid = $this->_controls_by_name[$name]->is_valid();
			if (!$valid) {
				$this->invalid_controls[] = $name;
			}
			$result = $valid && $result;
	 ...
	 */
	function get_renderer_() {
		$renderer = new AnewtFormRendererDefault;
		$renderer->set('form', $this);
		return $renderer;
	}

	/**
	 * Renders the form using the supplied renderer.
	 */
	function render() {
	...
	/**
	 * Returns this form as a rendered string.
	 */
	function to_string() {
		return to_string($this->render());
	}
