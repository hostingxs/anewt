...
class AnewtFormControlCheckbox extends AnewtFormControl
{
	...
	function build_widget()
	{
		...
		if ($this->get('extra_attributes')) {
			$attr = array_merge($attr, $this->get('extra_attributes'));
		}
		...
		} else
		{
			...
			if (!is_null($help))
			{
				$out->set_attribute('title', $help_text);
				$out->add_class('with-help');
			}
		}
...
class AnewtFormControlChoice extends AnewtFormControl
{
	 ...
	 *
	 * When multiple options have the same value, it will return the first.
	 *
	 * \param $value
	...
	function get_option($value) {
		foreach ($this->_options as $option_or_option_group) {
			$res = $option_or_option_group->get_option($value);
			if ($res)
				return $res;
		}
		return null;
...
#			$this->_ensure_selection();
	...
	function is_valid()
	{
		...
		if (!$this->_get('multiple') && is_null($values)) {
	 ...
	 *
	 * This is now an alias to add_option().
	 *
	 * \param $option_group
	 ...
	 * \private
	 *
	 * Checks if all options are disabled. For a single select control, this is
	 * the only valid case for having no options enabled.
	 *
	 * \return
	...
	function build_widget()
	{
		...
		if ($this->get('composite'))
		{
			...
			if ($this->_options)
				$this->_options[0]->setdefault('id', $id);
		...
		} else
		{
			...
			$out = new AnewtXHTMLSelect(null, $attr);
...
class AnewtFormOption extends Container
{
	...
	public function get_values() {
		return $this->_selected ? array($this->get('value')) : array();
	}

	/**
	 * Returns the given option.
	 *
	 * For the control returns $this if this value has the given value, or
	 * null otherwise.
	 *
	 * \param $value
	 *   The value of the option
	 ...
	 */
	function get_option($value) {
		return $this->get('value') == $value ? $this : null;
	}
	...
	public function set_value($values) {
		...
		$this->_selected = (!$this->get('disabled') && in_array($this->get('value'), $values));
	...
	public function _ensure_selection($force_first=false) {
		$this->_selected = (!$this->get('disabled') || $force_first);
	...
	function _build_checkbox()
	{
		...
		if ($this->get('extra_attributes')) {
			$input_attr = array_merge($input_attr, $this->get('extra_attributes'));
		}
...
class AnewtFormOptionGroup extends AnewtFormOption
{
	...
	public function set_value($values)
	{
		 ...
		 */
		if ($this->get('disabled')) $values = array();
	 ...
	 * \private
	 *
	 * Sets the \c id property of the first child.
	 *
	 * \param $id
	 *   The \c id property.
	 */
	...
	/**
	 * Cannot render as checkbox.
	 */
	public function _build_checkbox()
	{
		$label = $this->get('label');
		assert('is_string($label); // option group must have a string label');
		$label = ax_div_class($label, 'form-option-group-label');

		$option_group = ax_div_class($label, 'form-option-group');

		foreach ($this->_options as $option)
		{
			$option->_choice_control = $this->_choice_control;
			$option_group->append_child($option->_build_checkbox());
		}
	...
	public function _build_radiobutton()
	{
		...
		{
			$option->_choice_control = $this->_choice_control;
			$option_group->append_child($option->_build_radiobutton());
		}
...
/**
 * Form control for choices, always rendered as a select box.
 *
 * This is exactly an AnewtFormControlChoice, only the threshold
 * is set to 0, so that it will always be rendered as a dropdown
 * box. For details see AnewtFormControlChoice.
 *
 * \see AnewtFormControlChoice
 */
class AnewtFormControlSelect extends AnewtFormControlChoice {
	/**
	 * Create a new AnewtFormControlSelect instance.
	 *
	 * \param $name
	 *   The name of this control.
	 */
